...
if [ -z "$DB_EXISTS" ]; then
    if [ -f "/var/www/html/db_backup.sql" ]; then
        ...
        mysql --ssl=FALSE -h db -u "$WORDPRESS_DB_USER" -p"$WORDPRESS_DB_PASSWORD" "$WORDPRESS_DB_NAME" < /var/www/html/db_backup.sql
        
        echo ">> URL'ler güncelleniyor: $SITE_URL"
        mysql --ssl=FALSE -h db -u "$WORDPRESS_DB_USER" -p"$WORDPRESS_DB_PASSWORD" "$WORDPRESS_DB_NAME" <<EOF
