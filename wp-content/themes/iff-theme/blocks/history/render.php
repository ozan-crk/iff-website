...
<section class="<?php echo esc_attr($className); ?>" style="<?php echo esc_attr($style); ?>">
    <div class="container mx-auto">
        <div class="text-center max-w-3xl mx-auto mb-20">
            <h2 class="text-6xl font-custom font-bold text-warmgray mb-6 tracking-tighter uppercase italic"><?php echo esc_html($baslik); ?></h2>
            <?php if ($aciklama): ?>
            ...
            <?php if ($children): ?>
                ...
                ?>
                    <div class="group relative">
                        <a href="<?php echo esc_url($link); ?>" class="block relative aspect-[9/16] overflow-hidden modern-shadow hover-lift transition-all duration-500">
                            <?php if ($afis_url): ?>
                                <img src="<?php echo esc_url($afis_url); ?>" 
                                     alt="<?php echo esc_attr($h_baslik); ?>" 
                                     class="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-700">
                            <?php else: ?>
                                <div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-400 font-serif italic text-sm p-4 text-center">Öne Çıkan Görsel Yok</div>
                            <?php endif; ?>
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-6">
                                <span class="text-white text-xs font-bold uppercase tracking-widest mb-2 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">DETAYLARI GÖR</span>
                            </div>
                        </a>
                        
                        <div class="mt-4 text-center">
                            <h4 class="text-2xl font-custom font-bold text-warmgray group-hover:text-red transition-colors"><?php echo esc_html($h_baslik); ?></h4>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="col-span-full py-20 text-center border-4 border-dashed border-gray-300">
                    <p class="text-gray-400 font-serif italic">Henüz alt sayfa bulunamadı. (Seçilen Üst Sayfa ID: <?php echo $ust_sayfa_id; ?>)</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
