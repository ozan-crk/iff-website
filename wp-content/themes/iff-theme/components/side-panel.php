...
<aside id="side-panel"
    ...
    <div class="p-6 overflow-y-auto flex-1 custom-scrollbar">
        ...
        <?php if ($afis_aktif): ?>
            <!-- Güncel Afiş -->
            <div class="mb-8">
                <h4 class="text-xs font-bold text-red mb-3 uppercase tracking-widest"><?php echo esc_html($afis_baslik); ?>
                </h4>
        ...
        <?php if ($program_aktif): ?>
            <!-- Program Özeti -->
            <div class="mb-8">
                <h4 class="text-xs font-bold text-red mb-3 uppercase tracking-widest">
                    <?php echo esc_html($program_baslik); ?>
                </h4>

                <?php if (class_exists('IFF_Program_Manager')): ?>
                    <select id="panel-city-filter"
                        class="w-full mb-4 text-[10px] p-2 border-2 border-orange font-heading font-custom tracking-wider outline-none bg-white">
                        <option value="">TÜM ŞEHİRLER</option>
                        <?php
                        $active_cities = IFF_Program_Manager::get_active_cities();
                    ...
                    <script>
                        document.getElementById('panel-city-filter').addEventListener('change', function () {
                            const selectedCity = this.value;
                            const items = document.querySelectorAll('.side-prog-item');
                            let hasVisible = false;

                            items.forEach(item => {
                                if (!selectedCity || item.getAttribute('data-city') === selectedCity) {
                            ...
                            const emptyMsg = document.getElementById('side-prog-empty');
                            if (emptyMsg) {
                ...
                <div id="side-program-content" class="text-sm space-y-4">
                    ...
                    if (class_exists('IFF_Program_Manager')) {
                        ...
                        // Tüm şehirleri çekiyoruz, JS ile filtreleyeceğiz
                        $programs = IFF_Program_Manager::get_programs(50, $today, '', true);
                        ...
                        // Boş durum mesajı (JS ile kontrol edilecek)
                        echo '<p id="side-prog-empty" class="text-[10px] italic text-gray-400 font-serif" style="display: ' . ($programs ? 'none' : 'block') . ';">Bugün için uygun gösterim bulunmuyor.</p>';
                    }
                    ?>
                </div>
                <a href="<?php echo home_url('/program'); ?>"
                    class="mt-6 text-[10px] font-bold text-orange hover:underline block uppercase tracking-widest">TÜM
                    PROGRAM &rarr;</a>
            </div>
        <?php endif; ?>
        ...
        <div class="space-y-3">
            <?php if (have_rows('panel_hizli_butonlar', 'option')): ?>
                <?php while (have_rows('panel_hizli_butonlar', 'option')):
                    the_row();
            ...
            <?php else: ?>
                <!-- Varsayılan Butonlar -->
                <a href="#"
                    class="block bg-warmgray text-white p-3 text-center font-heading font-bold text-xs hover:bg-orange transition hover-lift">KATALOG
                    (PDF)</a>
                <a href="<?php echo home_url('/basvuru#gonullu'); ?>"
                    class="block bg-red text-white p-3 text-center font-heading font-bold text-xs hover:bg-orange transition hover-lift text-white no-underline">GÖNÜLLÜ
                    OL</a>
                <a href="#"
                    class="block border-2 border-warmgray p-3 text-center font-heading font-bold text-xs hover:bg-warmgray hover:text-white transition hover-lift text-warmgray">ESKİ
                    FİLMLER</a>
                <a href="<?php echo home_url('/basin'); ?>"
                    class="block border-2 border-red p-3 text-center font-heading font-bold text-xs hover:bg-red  hover:text-white transition hover-lift text-red uppercase">Basın
                    Bülteni</a>
            <?php endif; ?>
        </div>
    </div>
